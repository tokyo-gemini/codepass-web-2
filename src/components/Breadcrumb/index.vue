<template>
  <el-breadcrumb class="app-breadcrumb text-white" separator="/">
    <transition-group name="breadcrumb">
      <el-breadcrumb-item v-for="(item, index) in levelList" :key="item.path">
        <span
          ...
          <template v-else>{{ item.meta.title }}</template></span
        >
    ...
    methods: {
      getBreadcrumb() {
        ...
        if (!this.isDashboard(matched[0])) {
          matched = [{ path: '/performance', meta: { title: '首页' } }].concat(matched)
        }
      ...
      isDashboard(route) {
        ...
        if (!name) {
          return route && route.path === '/performance'
        }
        return name.trim() === 'Performance'
      },
      handleLink(item) {
